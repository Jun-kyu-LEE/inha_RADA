...
=== Stage 1: Health checks ===
  [PASS] ML  /health 200  | code=200 body={'status': 'ok', 'timestamp': '2026-05-26T20:59:02.269270'}
...
=== Stage 2: ML 서버 단독 /analyze 시나리오 4종 ===
  ...
  [PASS] gpu_mining: HTTP 200 + verdict 존재  | code=200
    ...
    gpu_mining category_signals: {"resource_abnormal": true, "network_abnormal": true, "system_abnormal": false, "sustained_minutes": 5, "triggered_patterns": ["GPU_MINING", "CONFIRMED_MINING", "GPU_CPU_IMBALANCE", "HIGH_GPU", "POOL_TRAFFIC", "OUTBOUND_DOMINANT", "MANY_EXT
  ...
  [PASS] cpu_only_mining: HTTP 200 + verdict 존재  | code=200
    ...
    cpu_only_mining category_signals: {"resource_abnormal": true, "network_abnormal": true, "system_abnormal": false, "sustained_minutes": 5, "triggered_patterns": ["CONFIRMED_MINING", "CPU_GPU_IMBALANCE", "HIGH_CPU", "MANY_EXTERNAL", "STRATUM_PATTERN"], "verdict_from_gating": 
...
=== Stage 3b: Spring /api/metrics → ML → DB 적재 시나리오 ===
  [PASS] spring_normal: 202 accepted  | code=202 body={'status': 'accepted', 'id': 2482, 'pcId': 'PC-E2E-001'}
  [PASS] spring_gpu_mining: 202 accepted  | code=202 body={'status': 'accepted', 'id': 2483, 'pcId': 'PC-E2E-001'}
  [PASS] spring_cpu_mining: 202 accepted  | code=202 body={'status': 'accepted', 'id': 2484, 'pcId': 'PC-E2E-001'}

=== Stage 4: DB 상태 검증 ===
    metrics_history rows: 15
    anomaly_history rows: 8
    ai_judgment_history rows: 8
  [PASS] metrics_history ≥ 3
  [PASS] anomaly_history ≥ 1 (HIGH/SUSPICIOUS 시나리오 1건+)
    anomaly_history latest: HIGH / HIGH_RISK | final=100.0 | cat={"system_abnormal": false, "network_abnormal": true, "resource_abnormal": true, "sustained_minutes": 2, "triggered_patterns": ["CONFIRMED_MINING", "CPU_GPU_IMBALANCE", "HIGH_CPU", "MANY_EXTERNAL", "STRAT
    scores JSONB 키 존재 (cat/final/retr): true,true,true
  [PASS] scores.category_signals 키 존재
  [PASS] scores.final 키 존재
  [PASS] scores.retrieval_evidence 키 존재 (warmup 성공)
...
=== Stage 5: Grafana 데이터소스/대시보드 점검 ===
  ...
  [PASS] Postgres datasource health OK  | code=200 body={'message': 'Database Connection OK', 'status': 'OK'}
    provisioned dashboards: ["RADA — PC Detail", "실습실 PC 자원 이상탐지 시스템 | LAB-01 관제"]
